...
8. Don't hand-roll "Continue with Google/Apple" buttons — the ONE brand-compliant set is `SocialAuthButtons` (`components/base/social-auth-buttons.tsx`). Never restyle it with the theme color, and render it on every auth surface (`/sign-in`, `/sign-up`)
9. Don't send an auth redirect anywhere but `${window.location.origin}/auth/callback` — that goes for OAuth `redirect_uri` and sign-up's `emailRedirectTo`. A bare origin strands the user on the marketing landing; a protected route races the auth guard and bounces them back to `/sign-in` still signed out
10. Don't hand-roll a logout in a demo shell. On `/demo/*` the leave affordance is "Exit demo" → `EXIT_DEMO_ROUTE`, decided by `useIsDemo()` (`src/lib/demo.ts`). It sits above Settings; the account row stays last
...
**Classy** — Lato + Inter fonts, warm beige background, red accent (#D11515), 0px radius (sharp corners). Elegant, editorial, refined.
