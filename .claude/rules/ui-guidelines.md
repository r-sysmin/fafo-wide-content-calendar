...
- **Sidebar footer holds the leave affordance, then settings, then the account row.**
  The order is fixed: the leave affordance sits ABOVE settings and the account/profile
  row, and the account row is always last. Which affordance you render is decided by
  `useIsDemo()` (`src/lib/demo.ts`) and nothing else — never re-derive demo-ness from
  the pathname in a component:
  - authenticated `/*` → "Log out", calls `signOut()` from AuthProvider;
  - public `/demo/*` → "Exit demo" with a close icon (`IconX`), a plain navigation to
    `EXIT_DEMO_ROUTE` (`/`). The demo has no session, so "Log out" is meaningless there.
  This app's footer lives in `src/pages/workspace/components/app-sidebar.tsx`
  (`PeekPaneBody`) and wires the shadcn sidebar primitives directly, because the footer
  also carries Settings and the signed-out Sign in / Create account items. Keep it that
  way; don't hand-roll a logout in a demo shell.
