...
function describeClickNode(node: EventTarget | null) {
  if (!(node instanceof HTMLElement)) return String(node)

  const label = node.dataset.peekDebug ? `[${node.dataset.peekDebug}]` : ""
  ...
  const aria = node.getAttribute("aria-label") ? `[aria=${node.getAttribute("aria-label")}]` : ""
  const text = node.textContent?.trim().replace(/\s+/g, " ").slice(0, 32)

  return `${node.tagName.toLowerCase()}${id}${classes}${role}${aria}${label}${text ? ` text="${text}"` : ""}`
}

function logSidebarClick(label: string, event: MouseEvent | React.MouseEvent<HTMLElement>, state: PeekState) {
  ...
  const ancestry = path
    ...
    .slice(0, 12)
    .map(describeClickNode)

  console.log(`[sidebar-click:${label}]`, {
    mode: state.isExpanded ? "expanded" : state.isOpen ? "peek" : "collapsed",
    point: { x: nativeEvent.clientX, y: nativeEvent.clientY },
    target: describeClickNode(target),
    currentTarget: describeClickNode(currentTarget),
...
export function PeekPane({ children }: { children: ReactNode }) {
  ...
  return (
    <div
      ...
      data-peek-debug="fixed-root"
      className="pointer-events-none fixed top-0 left-0 hidden h-screen shrink-0 grow-0 md:block"
      onClickCapture={(event) => logSidebarClick("fixed-root-capture", event, state)}
      style={{
        width: 0,
    ...
    >
      <div
        className="h-full"
        data-peek-debug="height-wrapper"
        onClickCapture={(event) => logSidebarClick("height-wrapper-capture", event, state)}
      >
        <div
          data-peek-debug="overflow-wrapper"
          className="pointer-events-none absolute inset-y-0 left-0 z-[9] flex w-0 flex-col overflow-visible"
          onClickCapture={(event) => logSidebarClick("overflow-wrapper-capture", event, state)}
          onMouseMove={handleOverflowMouseMove}
        >
          {/* The card — always mounted, slides between positions */}
          <div
            data-peek-debug="pane"
            className={cn(
              "group/sidebar-pane relative flex min-h-[200px] flex-col",
              SIDEBAR_PANE_CLASS,
            )}
            onClickCapture={(event) => logSidebarClick("pane-capture", event, state)}
            onClick={(event) => logSidebarClick("pane-bubble", event, state)}
            style={{
              width: state.width,
              height: paneHeight,
          ...
          >
            ...
            <div
              data-peek-debug="pane-spacer"
              className="pointer-events-auto relative bg-transparent"
              onClickCapture={(event) => logSidebarClick("pane-spacer-capture", event, state)}
              style={{
                top: -8,
                height: 8,
                width: state.width,
            ...
            />
            {/* Background card */}
            <div data-peek-debug="pane-background" className="absolute inset-0 z-[-1] rounded-lg bg-muted border border-sidebar-border shadow-lg" />
            {/* Sidebar content */}
            <div
              data-peek-debug="pane-content"
              className="flex h-full w-full flex-col overflow-hidden rounded-lg"
              onClickCapture={(event) => logSidebarClick("pane-content-capture", event, state)}
            >
              {children}
            </div>
          </div>
...
export function SidebarHoverArea({ children, className }: { children: ReactNode; className?: string }) {
  ...
  return (
    <div
      data-peek-debug="hover-area"
      onMouseEnter={handleHover}
      onMouseMove={handleHover}
      onMouseLeave={handleMouseLeave}
      onClickCapture={(event) => logSidebarClick("hover-area-capture", event, state)}
      onClick={(event) => logSidebarClick("hover-area-bubble", event, state)}
      className={className}
    >
      {children}
    </div>
...
export function useSidebarMouseListener() {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Pane sits at left:8, so right edge is width + 8. Add a small buffer so the
      // user can click items near the right edge without the listener closing the pane.
      const paneRight = state.width + 8
      const closeThreshold = Math.max(paneRight, peekX) + 24
      if (state.isOpen && !state.isExpanded && mouseX > closeThreshold) {
        console.log(`[mouse] past threshold x=${mouseX} > ${closeThreshold} → closePeek`)
        actions.closePeek()
      }
    }
    ...
    const handleMouseOut = (e: MouseEvent) => {
      clearTimeout(openTimerRef.current)
      openTimerRef.current = undefined
      if (!state.isOpen || state.isExpanded) return
      // `mouseout` with a null relatedTarget also fires when the cursor crosses an
      // iframe boundary (e.g. the Lovable preview). Only close when the cursor has
      // genuinely left the viewport rectangle — otherwise the pane snaps shut before
      // the user can click an item in it.
      const { clientX, clientY } = e
      const trulyLeftViewport =
        clientX <= 0 ||
        clientY <= 0 ||
        clientX >= window.innerWidth ||
        clientY >= window.innerHeight
      if (!e.relatedTarget && trulyLeftViewport) {
        console.log("[mouse] left viewport → closePeek")
        actions.closePeek()
      }
    }

    const handleClickCapture = (e: MouseEvent) => {
      const shouldLog = state.isOpen || state.isExpanded || e.clientX <= Math.max(state.width + 48, 320)
      if (!shouldLog) return
      logSidebarClick("window-capture", e, state)
    }
